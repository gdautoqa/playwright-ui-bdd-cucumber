...
When('I select {string} from the uploads folder', async function (fileName: string) {
  const filePath = path.join(process.cwd(), 'src', 'utils', 'uploads', fileName);
...
Then('the page should display {string}', async function (fileName: string) {
  await expect(this.fileUploadPage.uploadedFileNameLocator).toHaveText(fileName);
});