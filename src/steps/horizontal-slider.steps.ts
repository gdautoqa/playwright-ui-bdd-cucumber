...
When('I move the slider to the right until the value is {string}', async function (targetValue: string) {
  await this.horizontalSliderPage.moveSliderToValue(targetValue);
});

Then('I should see the slider value as {string}', async function (expectedValue: string) {
